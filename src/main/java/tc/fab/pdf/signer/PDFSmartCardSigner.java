...
import tc.fab.security.SmartCardAdapter;
...
public class PDFSmartCardSigner implements IPDFSigner {
	...
	keyStore = new SmartCardAdapter().login(handler);
	// TODO: alias selection
	alias = keyStore.aliases().nextElement();
	pkey = keyStore.getKey(alias, null);
	...
	ExternalSignature es = new PrivateKeySignature((PrivateKey) pkey,
		"SHA-1", null);
	...
	if (getOptions().hasDescription()) {
	    ...
	    String name = CertificateInfo.getSubjectFields(
		    (X509Certificate) keyStore.getCertificate(alias)).getField("CN");
