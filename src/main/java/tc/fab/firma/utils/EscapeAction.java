 ...
 * Before disposing of the window the Action will first attempt to hide any
 * popups. In this case the user will need to invoke the Escape key a second
 * time before the Window is disposed.
 */
public class EscapeAction extends AbstractAction {
	private static final long serialVersionUID = 7699289869218899662L;
	...
	/**
	 * Implement the Escape Action. First attempt to hide a popup menu. If no
	 * popups are found then dispose the window.
	 */
	@Override
	...
	public void register(JRootPane rootPane) {
		rootPane.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(ESCAPE_KEY_STROKE,
			KEY_STROKE_AND_KEY);
